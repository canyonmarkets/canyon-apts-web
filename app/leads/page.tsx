...
export default function LeadsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Switch to list after a beat so she can see it was added
    setTimeout(() => {
      setSaveStatus('idle');
      setTab('list');
    }, 1400);

    // Fire notification — silent fail so it never blocks the UX
  ...
  return (
    <div className="flex flex-col bg-iron-100 font-sans" style={{ height: '100dvh' }}>
            ...
            {visible.length > 0 && (
              <div className="px-4 space-y-3">
                {visible.map(lead => (
                  <div
                    key={lead.id}
                    className="bg-white rounded-2xl border border-iron-100 shadow-sm p-4"
                  >
                    {/* Top row: name + delete */}
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-base font-semibold text-iron-900 leading-tight">
                          {lead.name}
                        </p>
                    ...
                    {/* Call-back date + calendar button */}
                    {lead.callBackDate && (
      ...
      {/* ── Bottom navigation ───────────────────────────────────────────────── */}
      <nav className="shrink-0 bg-white border-t border-iron-200">
