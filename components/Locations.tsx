...
const LOCATIONS = [
  {
    city: 'Phoenix',
    // Phoenix is the canonical hub — no /apartments/phoenix spoke exists.
    href: '/furnished-apartments-phoenix',
    ...
    <section id="locations" className="bg-stone-50 px-6 py-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {LOCATIONS.map(({ city, href, description }, i) => (
            <Reveal key={city} delay={(i % 3) * 80} className="h-full">
              <Link href={href} className="block h-full" aria-label={`Furnished apartments in ${city}, AZ`}>
                <TiltCard
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-display font-bold text-2xl uppercase tracking-wide text-stone-900 group-hover:text-brand-600 transition-colors duration-300">
                      {city}
                    </h3>
                  </div>
                  <p className="text-stone-900 text-sm leading-relaxed">{description}</p>
                  <span className="mt-auto text-brand-600 text-xs font-semibold uppercase tracking-wide">
                    View {city} housing →
                  </span>
                </TiltCard>
              </Link>
